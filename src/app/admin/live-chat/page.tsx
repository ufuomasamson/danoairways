...
export default function AdminLiveChatPage() {
  ...
  useEffect(() => {
    async function fetchChats() {
      try {
        console.log('Admin: Fetching all chat users');
        const response = await fetch('/api/admin/chats');
        const data = await response.json();
        
        if (response.ok) {
          console.log('Admin: Found chat users:', data.users?.length || 0);
          setChats(data.users || []);
          if (!selectedUser && data.users?.length > 0) {
            setSelectedUser(data.users[0].user_auth_id);
          }
        } else {
          console.error('Admin: Error fetching chats:', data.error);
        }
      } catch (error) {
        console.error('Admin: Error fetching chats:', error);
      }
    }
    fetchChats();
  }, [selectedUser]);
  ...
  useEffect(() => {
    ...
    async function fetchMessages() {
      try {
        console.log('Admin: Fetching messages for user:', selectedUser);
        const response = await fetch(`/api/chat?user_auth_id=${selectedUser}`);
        const data = await response.json();
        
        if (response.ok) {
          console.log('Admin: Loaded messages:', data.messages?.length || 0);
          setMessages(data.messages || []);
        } else {
          console.error('Admin: Error fetching messages:', data.error);
        }
      } catch (error) {
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('Admin: Error sending message:', data.error);
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      <div className="w-64 bg-white p-4 border-r shadow-sm">
        <h2 className="font-bold mb-4 text-gray-800">Active Chats</h2>
        {chats.length === 0 ? (
      ...
      <div className="flex-1 flex flex-col">
        <div className="p-4 border-b bg-white shadow-sm">
          <h1 className="font-bold text-gray-800">Live Chat (Admin)</h1>
          {selectedUser && (
